...
struct BootView: View {
                ...
                VStack(spacing: 12) {
                    ...
                    GeometryReader { geo in
                        ZStack(alignment: .leading) {
                            ...
                            Capsule()
                                .fill(LinearGradient(colors: [Color.wosAccent, Color.wosAccentLight], startPoint: .leading, endPoint: .trailing))
    ...
    private func statusRow(icon: String, text: String, color: Color) -> some View {
        HStack(spacing: 8) {
            ...
            Text(text).font(.system(size: 13, weight: .medium)).foregroundColor(Color.wosTextSecondary)
        }
    }
    ...
    private var versionSection: some View {
        VStack(spacing: 4) {
            if let version = downloadService.currentVersion {
                ...
                Text("v\(WOSConstants.version) • Swift Native")
                    .font(.system(size: 11, design: .monospaced))
                    .foregroundColor(Color.wosTextDisabled)
            }
        }
        ...
        ZStack {
            ForEach(0..<3) { i in
                Circle()
                    ...
                    .offset(x: CGFloat(i % 2 == 0 ? -50 : 50), y: CGFloat(i * 30))
            }
        }
    }

    private func startBootAnimation() {
        withAnimation(.spring(response: 0.8, dampingFraction: 0.6).delay(0.2)) { logoOpacity = 1; logoScale = 1 }
        withAnimation(.easeInOut(duration: 1.0).delay(0.4)) { glowOpacity = 1 }
        withAnimation(.spring(response: 0.6, dampingFraction: 0.5).delay(0.3)) { ringOpacity = 1; ringScale = 1 }
        withAnimation(.linear(duration: 8).repeatForever(autoreverses: false).delay(0.5)) { rotation = 360 }
        withAnimation(.easeInOut(duration: 0.6).delay(0.6)) { textOpacity = 1 }
    }
