...
struct SetupView: View {
            ...
            VStack(spacing: 0) {
                ...
                GeometryReader { geo in
                    ZStack(alignment: .leading) {
                        ...
                        RoundedRectangle(cornerRadius: 3).fill(Color.wosAccent)
                ...
                VStack(spacing: 20) {
                    ...
                    if let errorMessage {
                        Text(errorMessage).foregroundColor(.wosDanger).font(.system(size: 13)).transition(.opacity)
                    }

                    Button(action: handleNext) {
                        HStack(spacing: 8) {
                            Text(step == totalSteps ? "Hoàn tất" : "Tiếp theo")
                            if step < totalSteps { Image(systemName: "arrow.right").font(.system(size: 12, weight: .bold)) }
                        }
                        ...
                        .padding(.vertical, 16)
                        .background(
                            LinearGradient(colors: [Color.wosAccent, Color.wosAccent.opacity(0.8)], startPoint: .leading, endPoint: .trailing)
                        )
                        .cornerRadius(14)
    ...
    private var progressValue: CGFloat { CGFloat(step) / CGFloat(totalSteps) }
    ...
    private var stepContent: some View {
            ...
            if !password.isEmpty {
                HStack(spacing: 4) {
                    ForEach(0..<6, id: \.self) { i in
                        Circle().fill(i < password.count ? Color.wosAccent : Color.white.opacity(0.2)).frame(width: 8, height: 8)
                    }
                    Text(passwordStrength).font(.system(size: 11)).foregroundColor(passwordStrengthColor).padding(.leading, 8)
                }
            }
        case 4:
        ...
        case 5:
            ...
            bootChoiceButton(title: "1. Tự boot drive riêng (Tự Build)", subtitle: "Đọc mã nguồn từ repo hoặc tải file để tự build", selected: isSelfBuildMode) {
                systemState.setBootDriveMode(.selfBuild(source: ""))
            }
            bootChoiceButton(title: "2. Chạy drive do Admin build", subtitle: "Dùng bản boot image admin đã build sẵn", selected: systemState.bootDriveMode == .adminBuilt) {
        ...
        case 6:
            if isSelfBuildMode {
                ...
                Button(action: { showFileImporter = true }) {
                    HStack { Image(systemName: "arrow.up.doc.fill"); Text("...hoặc tự tải lên file mã nguồn") }
    ...
    private func handleNext() {
        ...
        case 5:
            step = 6
            if !isSelfBuildMode { bootDrive.useAdminDrive { _ in } }
        case 6:
            if isSelfBuildMode {
