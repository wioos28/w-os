...
final class BootDriveService: ObservableObject {
    ...
    func clearCache() {
        defaults.removeObject(forKey: WOSConstants.Keys.bootDriveInfo)
        driveInfo = nil
        status = .idle
        progress = 0
    }
    ...
    private func saveDriveInfo(_ info: BootDriveInfo) {
        if let data = try? JSONEncoder().encode(info) {
            defaults.set(data, forKey: WOSConstants.Keys.bootDriveInfo)
        }
    }

    private func loadDriveInfo() {
        if let data = defaults.data(forKey: WOSConstants.Keys.bootDriveInfo),
